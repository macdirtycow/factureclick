//
//  InvoiceDOCXExportService.swift
//  Factureclick
//
...
struct InvoiceDOCXExportService {
    ...
    private func makeExportURL(invoice: Invoice) throws -> URL {
        ...
        return try exportFileStorageService.makeURL(fileName: "\(prefix)-\(invoice.invoiceNumber)-\(templateStyle.fileNameComponent).docx")
    }
    ...
    private func documentXML(
        ...
        let messageLines = [invoiceNotes, messageLine].filter { !$0.isEmpty }
        ...
        let layout = InvoiceDOCXTemplateLayout(style: templateStyle)
        ...
        var sections = [
            ...
            paragraph("", size: layout.spacingParagraphSize),
            heading(localization.invoiceLinesLabel, size: layout.headingSize, colorHex: layout.headingColorHex(accentHex: accentHex, inkHex: inkHex)),
            table(
                rows: [[localization.descriptionLabel, localization.qtyLabel, localization.unitLabel, localization.totalLabel]] + invoice.lines.map { line in
                    [
                        line.description,
            ...
            table(
                rows: [[localization.vatLabel, localization.vatAmountLabel]] + vatBreakdown.map {
                    [
            ...
            paragraph("", size: layout.spacingParagraphSize),
            heading(localization.paymentDetailsLabel, size: layout.headingSize, colorHex: layout.headingColorHex(accentHex: accentHex, inkHex: inkHex)),
            keyValueTable(
                rows: [
        ...
        if let partnerName = collaboration.partnerName {
            sections.append(paragraph("", size: layout.spacingParagraphSize))
            sections.append(heading(localization.collaborationLabel, size: layout.headingSize, colorHex: layout.headingColorHex(accentHex: accentHex, inkHex: inkHex)))
            sections.append(
                keyValueTable(
                    rows: [
        ...
        sections.append(paragraph("", size: layout.spacingParagraphSize))
        sections.append(heading(localization.termsLabel, size: layout.headingSize, colorHex: layout.headingColorHex(accentHex: accentHex, inkHex: inkHex)))
        sections.append(
            keyValueTable(
                rows: [
                    [localization.dueDateLabel, invoice.dueDate.formatted(date: .abbreviated, time: .omitted)],
    ...
    private func invoiceHeader(title: String, brandName: String, invoiceNumber: String, accentHex: String, inkHex: String, softAccentHex: String, logoRelationshipID: String?) -> String {
        ...
        case .classic:
            return logoPrefix(logoRelationshipID: logoRelationshipID, inkHex: inkHex) + headerTable(
                ...
                rows: [
                    cell(brandName, width: 6100, bold: true, textColor: inkHex, size: 24) + cell(invoiceNumber, width: 3100, bold: true, textColor: inkHex, alignment: "right", size: 22),
                    cell(title, width: 6100, bold: true, textColor: accentHex, size: 18) + cell(localization.invoiceNumberLabel, width: 3100, textColor: "9CA3AF", alignment: "right", size: 16)
                ]
            )
        case .premium:
            return logoPrefix(logoRelationshipID: logoRelationshipID, inkHex: inkHex) + headerTable(
                ...
                topBorderHex: accentHex,
                rows: [
                    cell(brandName, width: 5600, bold: true, shaded: softAccentHex, textColor: inkHex, size: 24) + cell(invoiceNumber, width: 3600, bold: true, shaded: accentHex, textColor: "FFFFFF", alignment: "right", size: 24),
                    cell(title, width: 5600, bold: true, textColor: accentHex, size: 18) + cell(localization.invoiceNumberLabel, width: 3600, textColor: "6B7280", alignment: "right", size: 16)
                ]
            )
        case .compact:
            return logoPrefix(logoRelationshipID: logoRelationshipID, inkHex: inkHex) + headerTable(
                ...
                topBorderHex: "CBD5E1",
                rows: [
                    cell("\(brandName) | \(title)", width: 6350, bold: true, shaded: "F8FAFC", textColor: inkHex, size: 18) + cell(invoiceNumber, width: 2850, bold: true, shaded: "FFFFFF", textColor: inkHex, alignment: "right", size: 18)
                ]
            )
        case .bold:
            return logoPrefix(logoRelationshipID: logoRelationshipID, inkHex: inkHex) + headerTable(
                ...
                topBorderHex: accentHex,
                rows: [
                    cell(title, width: 5900, bold: true, shaded: inkHex, textColor: "FFFFFF", size: 28) + cell(invoiceNumber, width: 3300, bold: true, shaded: accentHex, textColor: "FFFFFF", alignment: "right", size: 26),
                    cell(brandName, width: 5900, bold: true, shaded: inkHex, textColor: "FFFFFF", size: 20) + cell(localization.invoiceNumberLabel, width: 3300, shaded: inkHex, textColor: "FFFFFF", alignment: "right", size: 18)
                ]
            )
        }
    }
